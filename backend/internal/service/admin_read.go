...
func (s *AdminReadService) ModelsView(ctx context.Context) ([]map[string]any, error) {
	...
	for _, item := range items {
		out = append(out, map[string]any{
			...
			"max_reference_videos":  item.MaxReferenceVideos,
			"max_reference_audios":  item.MaxReferenceAudios,
			"max_reference_media":   item.MaxReferenceMedia,
			"supports_audio_output": item.SupportsAudioOutput,
