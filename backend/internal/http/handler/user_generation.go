...
// durationRange returns every whole-second duration in the provider's slider
// range. Some partner models (Kling 3 and Seedance 2) are not fixed-tier
// models: Adobe exposes a one-second-step slider, so advertising only a few
// hand-picked values makes our capability metadata inaccurate.
func durationRange(minSeconds, maxSeconds int) []string {
	if minSeconds <= 0 || maxSeconds < minSeconds {
		return []string{}
	}
	out := make([]string, 0, maxSeconds-minSeconds+1)
...
func (h *UserGenerationHandler) catalogEntries(c *gin.Context) ([]gin.H, error) {
	items := []gin.H{
		...
		{
			"id":                   "firefly-gpt-image-1.5",
			"provider":             "adobe",
			"type":                 "image",
			"ratios":               []string{"1:1", "3:2", "2:3"},
			"resolutions":          []string{"1K"},
			"image_to_image":       true,
			"max_reference_images": 6,
			"description":          "Adobe Firefly GPT Image 1.5",
		},
		{
		...
		},
		{
			...
			"provider":             "adobe",
			"type":                 "image",
			"ratios":               []string{"1:1", "3:2", "2:3", "4:3", "3:4", "5:4", "4:5", "9:16", "16:9", "21:9"},
			"resolutions":          []string{"1K"},
			"image_to_image":       true,
			"max_reference_images": 4,
			"description":          "Adobe Gemini 2.5 Nano Banana",
		},
		{
			...
			"image_to_image":       true,
			"max_reference_images": 6,
			"description":          "Adobe Gemini 3.0 Nano Banana Pro",
		},
		{
			"id":                   "firefly-nano-banana-2",
			"provider":             "adobe",
			"type":                 "image",
			"ratios":               []string{"1:1", "3:2", "2:3", "4:3", "3:4", "5:4", "4:5", "9:16", "16:9", "21:9"},
			"resolutions":          []string{"1K", "2K", "4K"},
			"image_to_image":       true,
			"max_reference_images": 6,
			"description":          "Adobe Gemini 3.1 Nano Banana 2",
		},
		{
			"id":             "firefly-image-5",
			"provider":       "adobe",
